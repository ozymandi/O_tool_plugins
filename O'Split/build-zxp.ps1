...
$stageDir = Join-Path $stageRoot 'OSplit'
$runtimeItems = @(
    'CSXS',
    'css',
    'host',
...
Write-Host ('[OSplit] Bundle ID: {0}' -f $bundleId)
Write-Host ('[OSplit] Bundle version: {0}' -f $bundleVersion)
Write-Host ('[OSplit] Staging runtime files in: {0}' -f $stageDir)
Write-Host ('[OSplit] Building signed package: {0}' -f $zxpPath)
...
Write-Host '[OSplit] ZXP build complete.'
Write-Host ('[OSplit] Output: {0}' -f $zxpPath)
