...
$stageDir = Join-Path $stageRoot 'OSelect'
$runtimeItems = @(
    'CSXS',
    'css',
    'host',
...
Write-Host ('[OSelect] Bundle ID: {0}' -f $bundleId)
Write-Host ('[OSelect] Bundle version: {0}' -f $bundleVersion)
Write-Host ('[OSelect] Staging runtime files in: {0}' -f $stageDir)
Write-Host ('[OSelect] Building signed package: {0}' -f $zxpPath)
...
Write-Host '[OSelect] ZXP build complete.'
Write-Host ('[OSelect] Output: {0}' -f $zxpPath)
