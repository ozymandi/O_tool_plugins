...
if (-not $bundleVersion) { throw 'Could not read ExtensionBundleVersion from manifest.xml' }
...
$stageDir = Join-Path $stageRoot 'OAlign'
...
if (-not $safeName) { $safeName = 'OAlign' }
...
Write-Host ('[OAlign] Bundle ID: {0}' -f $bundleId)
Write-Host ('[OAlign] Bundle version: {0}' -f $bundleVersion)
Write-Host ('[OAlign] Building signed package: {0}' -f $zxpPath)

& $signTool @signArgs

if (-not (Test-Path -LiteralPath $zxpPath)) { throw "ZXPSignCmd did not produce the expected file: $zxpPath" }
if (-not $KeepStage) { Remove-Item -LiteralPath $stageRoot -Recurse -Force }

Write-Host '[OAlign] ZXP build complete.'
Write-Host ('[OAlign] Output: {0}' -f $zxpPath)
