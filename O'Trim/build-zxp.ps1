...
$stageDir = Join-Path $stageRoot 'OTrim'
$runtimeItems = @(
    'CSXS',
    'css',
    'host',
...
Write-Host ('[OTrim] Bundle ID: {0}' -f $bundleId)
Write-Host ('[OTrim] Bundle version: {0}' -f $bundleVersion)
Write-Host ('[OTrim] Staging runtime files in: {0}' -f $stageDir)
Write-Host ('[OTrim] Building signed package: {0}' -f $zxpPath)
...
Write-Host '[OTrim] ZXP build complete.'
Write-Host ('[OTrim] Output: {0}' -f $zxpPath)
