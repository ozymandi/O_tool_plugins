...
param(
    [string]$ExtensionName = "OLineArray",
...
function Enable-CepDebugMode {
    ...
    if (Test-Path -LiteralPath $adobeKey) {
        $csxsKeys = Get-ChildItem -LiteralPath $adobeKey | Where-Object {
            $_.PSChildName -match "^CSXS\.\d+$"
        }
    }
    ...
    if ($csxsKeys.Count -eq 0) {
        ...
        if (-not (Test-Path -LiteralPath $fallback)) {
            New-Item -Path $fallback -Force | Out-Null
        }
    ...
    foreach ($key in $csxsKeys) {
        ...
        New-ItemProperty -Path $targetKey -Name "PlayerDebugMode" -Value "1" -PropertyType String -Force | Out-Null
    }
}
