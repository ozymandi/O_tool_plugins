...
$stageDir = Join-Path $stageRoot 'ODeselect'
$runtimeItems = @(
    'CSXS',
    'css',
    'host',
...
foreach ($item in $runtimeItems) {
    ...
    if (-not (Test-Path -LiteralPath $source)) {
        throw "Required runtime item missing: $source"
    }
...
if ($TimestampUrl) {
    $signArgs += @('-tsa', $TimestampUrl)
}

Write-Host ('[ODeselect] Bundle ID: {0}' -f $bundleId)
Write-Host ('[ODeselect] Bundle version: {0}' -f $bundleVersion)
Write-Host ('[ODeselect] Staging runtime files in: {0}' -f $stageDir)
Write-Host ('[ODeselect] Building signed package: {0}' -f $zxpPath)

& $signTool @signArgs

if (-not (Test-Path -LiteralPath $zxpPath)) {
    throw "ZXPSignCmd did not produce the expected file: $zxpPath"
}
...
Write-Host '[ODeselect] ZXP build complete.'
Write-Host ('[ODeselect] Output: {0}' -f $zxpPath)
