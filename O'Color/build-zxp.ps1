...
$stageDir = Join-Path $stageRoot 'OColor'
...
if (-not $safeName) { $safeName = 'OColor' }
...
Write-Host ('[OColor] Building signed package: {0}' -f $zxpPath)
& $signTool @signArgs
...
Write-Host '[OColor] ZXP build complete.'
Write-Host ('[OColor] Output: {0}' -f $zxpPath)
