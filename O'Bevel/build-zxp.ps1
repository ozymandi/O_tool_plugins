...
$stageDir = Join-Path $stageRoot 'OBevel'
...
if (-not $safeName) { $safeName = 'OBevel' }
...
Write-Host ('[OBevel] Building signed package: {0}' -f $zxpPath)
& $signTool @signArgs
...
Write-Host '[OBevel] ZXP build complete.'
Write-Host ('[OBevel] Output: {0}' -f $zxpPath)
