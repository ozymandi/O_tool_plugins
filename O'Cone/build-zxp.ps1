...
$stageDir = Join-Path $stageRoot 'OCone'
...
if (-not $safeName) { $safeName = 'OCone' }
...
Write-Host ('[OCone] Building signed package: {0}' -f $zxpPath)
& $signTool @signArgs
...
Write-Host '[OCone] ZXP build complete.'
Write-Host ('[OCone] Output: {0}' -f $zxpPath)
