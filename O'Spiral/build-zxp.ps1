...
$stageDir = Join-Path $stageRoot 'OSpiral'
...
if (-not $safeName) { $safeName = 'OSpiral' }
...
Write-Host ('[OSpiral] Building signed package: {0}' -f $zxpPath)
& $signTool @signArgs
...
Write-Host '[OSpiral] ZXP build complete.'
Write-Host ('[OSpiral] Output: {0}' -f $zxpPath)
