...
$stageDir = Join-Path $stageRoot 'OConnect'
$runtimeItems = @(
    'CSXS',
    'css',
    'host',
...
Write-Host ('[OConnect] Bundle ID: {0}' -f $bundleId)
Write-Host ('[OConnect] Bundle version: {0}' -f $bundleVersion)
Write-Host ('[OConnect] Staging runtime files in: {0}' -f $stageDir)
Write-Host ('[OConnect] Building signed package: {0}' -f $zxpPath)
...
Write-Host '[OConnect] ZXP build complete.'
Write-Host ('[OConnect] Output: {0}' -f $zxpPath)
