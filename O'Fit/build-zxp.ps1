...
$stageDir = Join-Path $stageRoot 'OFit'
...
Write-Host ('[OFit] Bundle ID: {0}' -f $bundleId)
Write-Host ('[OFit] Building signed package: {0}' -f $zxpPath)
...
Write-Host '[OFit] ZXP build complete.'
Write-Host ('[OFit] Output: {0}' -f $zxpPath)
