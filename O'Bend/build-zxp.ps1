...
$stageDir = Join-Path $stageRoot 'OBend'
...
if (-not $safeName) { $safeName = 'OBend' }
...
Write-Host ('[OBend] Building signed package: {0}' -f $zxpPath)
& $signTool @signArgs
...
Write-Host '[OBend] ZXP build complete.'
Write-Host ('[OBend] Output: {0}' -f $zxpPath)
