(function () {
    var STORAGE_KEY = "overtex.panel.settings.v1";
    ...
    function getSnapshot() {
        return { strokeWidth: fields.strokeWidth.value };
    }
    function saveSettings() { safeStorageSet(JSON.stringify(getSnapshot())); }

    function applySnapshot(snap) {
        fields.strokeWidth.value = String(snap.strokeWidth);
    }

    function restoreSettings() {
        var restored = { strokeWidth: "0.1" };
    ...
    function collectConfig() {
        var sw = parseFloat(fields.strokeWidth.value);
        if (!Number.isFinite(sw) || sw <= 0) sw = 0.1;
    ...
    async function runExtract() {
        ...
        saveSettings();
        setBusy(true);
        setStatus("info", "Extracting vertices...");
        try {
            var payload = escapeForEval(JSON.stringify(cfg));
            var response = parseHostResponse(await evalHost("overtexRun('" + payload + "')"));
    ...
    async function initializePanel() {
        try {
            var handshake = parseHostResponse(await evalHost("overtexHandshake()"));
    ...
    fields.strokeWidth.addEventListener("input", saveSettings);
    fields.strokeWidth.addEventListener("change", saveSettings);
    buttons.extract.addEventListener("click", runExtract);
