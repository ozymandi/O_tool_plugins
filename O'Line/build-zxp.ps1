...
$stageDir = Join-Path $stageRoot 'OLine'
...
if (-not $safeName) { $safeName = 'OLine' }
...
Write-Host ('[OLine] Building signed package: {0}' -f $zxpPath)
& $signTool @signArgs
...
Write-Host '[OLine] ZXP build complete.'
Write-Host ('[OLine] Output: {0}' -f $zxpPath)
